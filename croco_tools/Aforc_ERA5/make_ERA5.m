...
for Y=Ymin:Ymax
    ...
    for M=mo_min:mo_max
        ...
        %
        %-------------------------------------------------------------------%
        nc=netcdf([ERA5_dir,'Q_Y',num2str(Y),'M',num2str(M),'.nc']);
        ERA5_time=nc{'time'}(:);
        close(nc);
        ...
        if exist(fname)==0
            ...
            if makefrc==1
                for aa=1:itolap
                    ...
                    nc_frc{'shf_time'}(aa)=nc{'time'}(tndx-(itolap-aa));
                    nc_frc{'swf_time'}(aa)=nc{'time'}(tndx-(itolap-aa));
                    nc_frc{'sst_time'}(aa)=nc{'time'}(tndx-(itolap-aa));
                    nc_frc{'sss_time'}(aa)=nc{'time'}(tndx-(itolap-aa));
                    nc_frc{'srf_time'}(aa)=nc{'time'}(tndx-(itolap-aa));
                    nc_frc{'wwv_time'}(aa)=nc{'time'}(tndx-(itolap-aa));
                end
            end
            if makeblk==1
                for aa=1:itolap
